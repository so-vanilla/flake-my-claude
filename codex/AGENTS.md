...
- Every `$route-work` normal/check/status/resume result includes concise next-step model advice: recommended role/model and effort, reason, and any required user or harness action. It is informational only: do not switch the root, edit `config.toml`, or make an extra model call. For a valid AI-DLC/Superpowers selection, defer model routing to that lifecycle and name its next entry instead of applying this global policy.
