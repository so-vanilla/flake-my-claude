...
以下を含むこと:
- モジュール分割とコンポーネント構成
- インターフェース定義（モジュール間の型・契約。関数シグネチャ・データ構造を具体的に記述）
- ファイル所有権の割り当て案
- データフロー
...
### Step 13: 設計プラン生成 + スケルトン + 検証
Readツールで `~/.claude/team/design-finalize-guide.md` を読み、以下を順に実行する:
1. 設計プラン生成 → `/tmp/claude-team/${MASTER_TEAM_ID}/design-plan.md`
2. スケルトンファイル生成 → `/tmp/claude-team/${MASTER_TEAM_ID}/skeletons/`
3. テスト項目の整合性検証
