...
class ObjectiveTransitionCompatibilityTests(unittest.TestCase):
    ...
    def test_evidence_six_path_aggregate_is_compact_canonical_json_without_lf(self):
        ...
        # This receipt remains evidence of the original accepted source bytes.
        # The explicit successor binds current bytes and its own actual tests.
        from ai_agent_workflow.runtime_source_extension import verify_runtime_source_extension
        current = verify_runtime_source_extension(ROOT.parent)
        self.assertEqual(set(evidence["bound_product_digests"]), set(current))
        physical_digests = evidence["bound_product_digests"]
