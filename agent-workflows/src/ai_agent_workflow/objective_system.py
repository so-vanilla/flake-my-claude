...
class ObjectiveSystemV1:
    ...
    def capture_intake(self, inputs, authority, expected_head):
        return self.compile("group.B.B1", inputs, authority, expected_head)

    def discover_context(self, inputs, authority, expected_head):
        return self.compile("group.B.B2", inputs, authority, expected_head)

    def classify_scope(self, inputs, authority, expected_head):
        return self.compile("group.B.B3", inputs, authority, expected_head)

    def continue_inquiry(self, inputs, authority, expected_head):
        return self.compile("group.B.B4", inputs, authority, expected_head)

    def propose_options(self, inputs, authority, expected_head):
        return self.compile("group.B.B5", inputs, authority, expected_head)

    def assess_feasibility(self, inputs, authority, expected_head):
        return self.compile("group.B.B6", inputs, authority, expected_head)

    def prepare_approval(self, inputs, authority, expected_head):
        return self.compile("group.B.B7", inputs, authority, expected_head)
    ...
    def compile(self, qualified_id: str, inputs: Mapping[str, Any], authority: Mapping[str, Any], expected_head: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if qualified_id == "group.B.B3" and (values.get("owner") == "unknown" or (values.get("operation") == "quick" and values.get("reversibility") == "risky")):
            return self._refusal(qualified_id, "blocked_missing_authority", auth, head)
        if qualified_id == "group.B.B4":
            unknowns = values.get("material_unknowns", [])
            if (unknowns == [] and values.get("inquiry_complete") is True
                    and isinstance(values.get("resolution_reason"), str)
                    and values["resolution_reason"].strip()
                    and isinstance(values.get("resolution_refs"), list)
                    and values["resolution_refs"]
                    and all(self._physical(ref) for ref in values["resolution_refs"])):
                return self._artifact(qualified_id, values, auth, head, "B4.inquiry-resolved")
    ...
    def _approval(self, values: Mapping[str, Any], authority: Mapping[str, Any], head: Mapping[str, Any]) -> dict[str, Any]:
        ...
        runtime = values.get("approval_scope") == "project-local"
        source = "mock" if runtime and values.get("mode") == "rehearsal" else "human"
        if not isinstance(receipt, Mapping) or receipt.get("source") != source or receipt.get("explicit") is not True or receipt.get("decision") != "approve":
            return self._refusal("group.B.B7", "blocked_implicit_approval", authority, head)
        if runtime:
            eligible = values.get("mode") in {"real", "rehearsal"} and isinstance(values.get("namespace"), str) and values["namespace"].startswith("project:")
        else:
            eligible = values.get("approval_scope") == "fixture-only" and isinstance(values.get("namespace"), str) and values["namespace"].startswith("fixture:")
        if not eligible:
    ...
    def _command(self, values: Mapping[str, Any], authority: Mapping[str, Any], head: Mapping[str, Any]) -> dict[str, Any]:
        selector = "B7.explicit-project-approval" if values.get("approval_scope") == "project-local" else "B7.explicit-fixture-approval"
        inputs, candidate = self._parts(values, selector)
