    ...
    *,
    ...
    for item in refs:
        ...
        if _source_digest(path) != item.get("digest"):
            extension_verifier = _source_file(root, "agent-workflows/src/ai_agent_workflow/runtime_source_extension.py")
            if _source_digest(extension_verifier) != _RUNTIME_EXTENSION_VERIFIER_DIGEST:
                raise ValueError("current runtime source-extension verifier drifted")
            if not is_extended_historical_source(root, item["path"], item.get("digest")):
                raise ValueError("%s digest does not bind physical bytes" % label)
