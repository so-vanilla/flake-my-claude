...
class ControlKernel:
    ...
    def _validate_object_payload(self, object_type: str, payload: Any) -> None:
        ...
        if object_type == "objective":
            ...
            if set(payload) != required or not isinstance(payload.get("outcomes"), list):
    ...
    def _validate_objective_associations(self, state: Mapping[str, Any], load_objects: bool) -> None:
        runtime = state.get("metadata", {}).get("runtime_identity")
        if runtime is not None:
            self._validate_runtime_identity(runtime)
            if runtime != state.get("authority", {}).get("runtime_identity"):
                raise IntegrityBlockedError("runtime identity differs from genesis authority")
        ...
        for index, event in enumerate(events):
            ...
            if load_objects:
                ...
                if approval.get("approval_scope") == "project-local":
                    self._validate_runtime_approval_binding(runtime, approval, candidate)
                    if any(event.get(key) != approval.get(key) for key in ("run_id", "namespace", "approval_scope")):
                        raise IntegrityBlockedError("runtime approval event identity is cross-wired")
                elif runtime is not None:
                    raise IntegrityBlockedError("runtime cannot contain fixture approvals")
    ...
    def _validate_state(self, state: Mapping[str, Any], load_objects: bool = True) -> None:
        ...
        for task_id, task in state["tasks"].items():
            ...
            live_lease_statuses = ("running", "leased", "blocked_review", "fix_claimed")
            ...
            if isinstance(lease, Mapping):
                if task.get("lease_status") != "leased" or task_status not in live_lease_statuses:
    ...
    def _empty_state(
        ...
        if runtime_identity is not None:
            self._validate_runtime_identity(runtime_identity)
            if fixture_identity is not None:
                raise AuthorizationError("runtime and fixture identities cannot coexist")
            state["metadata"]["runtime_identity"] = _copy(runtime_identity)
        if fixture_identity is not None:
            expected_fixture = {
    ...
    def _validate_live_task_authority(
        ...
        *,
        ...
        if state.get("metadata", {}).get("runtime_identity") is not None:
            grant = state["metadata"].get("operational_task_grant")
            if not isinstance(grant, Mapping) or state.get("group", {}).get("id") != "E":
                raise AuthorizationError("operational Task requires the accepted D to E execution grant")
            if self._operational_execution_grant(state, grant.get("bundle_ref")) != grant:
                raise AuthorizationError("operational execution grant differs from its D bundle")
            planned = grant["tasks"].get(task.get("task_id"))
            if (not isinstance(planned, Mapping) or set(task_scope) != set(planned["write_scope"])
                    or task.get("output_path") != planned["report_path"]):
                raise AuthorizationError("Task is outside its exact approved D8/D10 plan")
    ...
    def _ensure_durable_payload(
        ...
        if isinstance(value, Mapping):
            for key, child in value.items():
                ...
                schema_owned_attempt_authority_ref = (
                    raw_key == "attempt_authority_ref"
                ...
                if schema_owned_attempt_authority_ref:
                    try:
                        ...
                        raise AuthorizationError(
                            "%s.%s is not a structured object reference" % (label, key)
                        ) from exc
                if (
                    not identifier_map
    ...
    def _operational_execution_grant(self, state: Mapping[str, Any], bundle_ref: Any) -> Dict[str, Any]:
        """Derive finite task authority only from actual closed D artifacts."""
        from .planning_system import PlanningSystemV1
        bundle = self._load_ref_object(bundle_ref, "closed D bundle")["payload"]
        if (bundle.get("group_id") != "D" or bundle.get("run_id") != self.run_id
                or bundle.get("unresolved_items") or bundle.get("invalidated_artifacts")
                or not bundle.get("acceptance_evidence")):
            raise AuthorizationError("execution grant requires an accepted closed D bundle")
        available = {item["path"]: item["digest"] for item in bundle["canonical_artifacts"]}
        outputs = {}
        identity = state["metadata"]["runtime_identity"]
        for local in ("D8", "D10", "D12"):
            artifact_id = "runtime-" + local
            artifact = state["artifacts"].get(artifact_id, {})
            if available.get(artifact.get("path")) != str(artifact.get("digest", "")).removeprefix("sha256:"):
                raise AuthorizationError("closed D bundle omits its exact planning artifact")
            value = self._load_ref_object(artifact["object_ref"])["payload"]["payload"]
            compiled = value.get("compiled", {})
            qualified = "group.D." + local
            if (value.get("qualified_id") != qualified or value.get("runtime_identity") != identity
                    or value.get("objective_digest") != state["objective_ref"]["digest"]
                    or compiled.get("qualified_id") != qualified
                    or compiled.get("schema") != "planning-system-artifact/v1"
                    or PlanningSystemV1().compile(qualified, value["inputs"], compiled["authority"], compiled["expected_head"]) != compiled):
                raise AuthorizationError("D planning artifacts are stale or not compiler accepted")
            outputs[local] = compiled["output"]
        if outputs["D12"].get("status") not in {"ready", "ready_with_accepted_risks"}:
            raise AuthorizationError("D12 has not passed implementation readiness")
        tasks = outputs["D8"].get("tasks", [])
        briefs = outputs["D10"].get("briefs", [])
        if (not tasks or len({t["task_id"] for t in tasks}) != len(tasks)
                or len({b["task_id"] for b in briefs}) != len(briefs)
                or {t["task_id"] for t in tasks} != {b["task_id"] for b in briefs}):
            raise AuthorizationError("D8 and D10 must contain the same exact planned tasks")
        by_id = {b["task_id"]: b for b in briefs}
        planned = {}
        for task in tasks:
            brief = by_id[task["task_id"]]
            files = self._scope_paths(task["files"], "planned files")
            if set(files) != set(self._scope_paths(brief["write_scope"], "planned brief files")) or task["report_path"] != brief["report_path"]:
                raise AuthorizationError("D8 files and D10 write scopes differ")
            scopes = sorted(set(files + self._scope_paths(task["report_path"], "planned report")))
            if any("*" in scope or self.repository_root not in (self.repository_root / scope).resolve().parents for scope in scopes):
                raise AuthorizationError("planned writes must be concrete project-local files")
            planned[task["task_id"]] = {"write_scope": scopes, "report_path": task["report_path"]}
        return {"schema": "operational-task-grant/v1", "bundle_ref": _copy(bundle_ref), "objective_digest": state["objective_ref"]["digest"], "tasks": planned}

    def _validate_runtime_identity(self, identity: Any) -> None:
        fields = {"schema", "run_id", "namespace", "approval_scope", "project_root", "mode"}
        if (not isinstance(identity, Mapping) or set(identity) != fields
                or identity.get("schema") != "project-local-run-identity/v1"
                or identity.get("run_id") != self.run_id
                or identity.get("namespace") != "project:" + str(self.run_id)
                or identity.get("approval_scope") != "project-local"
                or identity.get("mode") not in {"real", "rehearsal"}
                or not isinstance(identity.get("project_root"), str)):
            raise AuthorizationError("invalid project-local runtime identity")
        project = Path(identity["project_root"])
        if not project.is_absolute() or str(project.resolve()) != str(project):
            raise AuthorizationError("runtime project must be an absolute canonical path")
        if self._direct_path is not None or project != self.repository_root:
            raise AuthorizationError("runtime project must match the Kernel repository root")

    def _validate_runtime_approval_binding(self, identity: Any, approval: Mapping[str, Any], candidate: Mapping[str, Any]) -> None:
        self._validate_runtime_identity(identity)
        if any(approval.get(key) != identity[key] for key in ("run_id", "namespace", "approval_scope", "project_root", "mode")):
            raise AuthorizationError("approval is bound to another runtime identity")
        if candidate.get("namespace") != identity["namespace"]:
            raise AuthorizationError("candidate is bound to another runtime namespace")
        project, candidate_path = Path(identity["project_root"]), Path(candidate["path"])
        if (not candidate_path.is_absolute() or str(candidate_path.resolve()) != str(candidate_path)
                or project not in candidate_path.parents):
            raise AuthorizationError("candidate must be within its canonical project")
    ...
    def _validate_objective_approval_payload(approval: Any) -> None:
        ...
        operational = isinstance(approval, Mapping) and approval.get("approval_scope") == "project-local"
        if operational:
        ...
        if operational:
            receipt_fields |= {"project_root", "mode"}
        source = "mock" if operational and approval.get("mode") == "rehearsal" else "human"
        if (approval.get("schema") != "objective-approval/v1"
                or approval.get("decision") != "approve"
                or approval.get("approval_scope") != ("project-local" if operational else "fixture-only")
                or not isinstance(approval.get("namespace"), str)
                or not approval["namespace"].startswith("project:" if operational else "fixture:")
                or not isinstance(actor, Mapping) or set(actor) != {"kind", "actor_id"}
                or actor.get("kind") != source
                ...
                or receipt.get("schema") != ("mock-approval-receipt/v1" if source == "mock" else "human-approval-receipt/v1")
        ...
        if operational and (approval.get("mode") not in {"real", "rehearsal"}
                or not isinstance(approval.get("project_root"), str)
                or any(receipt.get(key) != approval.get(key) for key in ("project_root", "mode"))):
            raise AuthorizationError("runtime receipt project/mode binding is invalid")
    ...
    def _validate_payload_shape(self, operation: str, payload: Any, *, verify_physical_attestation: bool = True) -> None:
        ...
        fields = {
            ...
            "open_operational_group": {"group_id", "epoch_id", "input_ref"},
        ...
        if operation == "entry":
            ...
            if (not isinstance(candidate, Mapping) or set(candidate) != {"path", "version", "digest", "namespace"}
                    ...
                    or not isinstance(candidate.get("namespace"), str) or not candidate["namespace"].startswith(("fixture:", "project:"))):
            ...
            if not isinstance(payload["outcomes"], list):
                raise CommandValidationError("validate_findings.outcomes must be an array")
        ...
        elif operation in {"open_epoch", "open_operational_group"}:
            if operation == "open_operational_group":
                _id(payload.get("group_id"), "group_id")
            if not isinstance(payload["epoch_id"], str) or not _ID.fullmatch(payload["epoch_id"]):
    ...
    def _validate_command(
        ...
        if operation in {"open_epoch", "open_review_epoch", "open_operational_group"} and command.get("input_refs") != [payload.get("input_ref")]:
        ...
        if operation == "approve_objective":
            ...
            operational = approval.get("approval_scope") == "project-local"
            if operational:
                expected_authority_fields |= {"project_root", "mode"}
                self._validate_runtime_approval_binding(state.get("metadata", {}).get("runtime_identity"), approval, candidate)
                if any(authority.get(key) != approval.get(key) for key in ("project_root", "mode")):
                    raise AuthorizationError("runtime approval authority project/mode mismatch")
                path = Path(candidate["path"])
                if not path.is_file() or _digest_bytes(path.read_bytes()) != candidate["digest"]:
                    raise AuthorizationError("runtime objective physical bytes are stale")
            ...
            if (set(authority) != expected_authority_fields
                    ...
                    or authority.get("approval_scope") != ("project-local" if operational else "fixture-only")
            ...
            if not operational and fixture_identity != expected_fixture:
    ...
    def _generic_reducer(self, state: Dict[str, Any], command: Mapping[str, Any], stage: Optional[Path]) -> Dict[str, Any]:
        ...
        if operation == "open_operational_group":
            ...
            self._validate_runtime_identity(identity)
            authority = command["authority_ref"]
            protected = ["group", "epoch", "ready"]
            if (authority.get("runtime_identity") != identity
                    or authority.get("scopes") != [operation]
                    or authority.get("write_scopes") != [identity["namespace"]]
                    or authority.get("protected_fields") != protected
                    or command.get("protected_fields") != protected
                    or command.get("scope") != [identity["namespace"]]):
                raise AuthorizationError("operational Group transition authority is invalid")
            associations = state.get("objective_approvals", {})
            if not associations:
                raise AuthorizationError("operational Group requires an approved objective")
            approval = self._load_ref_object(state["objective_ref"]["approval_ref"])["payload"]
            if authority.get("human_receipt") != approval["receipt"]:
                raise AuthorizationError("operational Group transition requires the current objective receipt")
            next_group = {"B": "C", "C": "D", "D": "E", "E": "H"}.get(state["group"]["id"])
            if (state.get("status") != "paused_after_group"
                    or state["group"].get("status") != "closed"
                    or state["epoch"].get("status") != "closed"
                    or payload["group_id"] != next_group
                    or state.get("leases")
                    or payload["input_ref"] != state["group"].get("bundle_ref")):
                raise LifecycleClosedError("operational Group requires its immediate closed predecessor bundle")
            self._validate_input_ref_binding(state, payload["input_ref"], "operational Group input")
            epoch_id = payload["epoch_id"]
            history = state["metadata"].setdefault("operational_group_history", [])
            used_epochs = {item["epoch"]["id"] for item in history} | set(state["epoch_contexts"])
            if epoch_id in used_epochs:
                raise LifecycleClosedError("operational Group requires a fresh Epoch ID")
            if next_group == "E":
            ...
            state["epoch_contexts"] = {epoch_id: {**state["epoch"], "started_at_revision": state["revision"] + 1, "input_ref": _copy(payload["input_ref"])}}
            state["status"] = "active"
            return state
        ...
        if operation == "approve_objective":
            ...
            if not blockers:
                ...
                if task and task.get("status") in ("blocked_review", "fix_claimed"):
                    # A split post-E6 repair deliberately keeps the original
                    # Worker lease through Finding closure.  Closure clears
                    # the review block but cannot declare the Work Product
                    # successful before accept_task_result persists it.
                    task["status"] = (
                        "leased"
                        if isinstance(state.get("leases", {}).get(task["task_id"]), Mapping)
                        else "succeeded"
                    )
        elif operation in ("invalidate_task", "replan_task"):
            task_id = payload["task_id"]
    ...
    def open_operational_group(self, group_id: str, epoch_id: str, input_ref: Mapping[str, Any], *, authority_ref: Mapping[str, Any], idempotency_key: Optional[str] = None) -> Dict[str, Any]:
        """Open the immediate operational successor from its accepted closure."""
        return self.apply(self._command_for(
            "open_operational_group", {"group_id": group_id, "epoch_id": epoch_id, "input_ref": _copy(input_ref)},
            authority_ref=authority_ref, input_refs=[input_ref], idempotency_key=idempotency_key,
            protected_fields=["group", "epoch", "ready"], scope=[authority_ref["runtime_identity"]["namespace"]],
        ))
