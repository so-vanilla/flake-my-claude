...
description: Report the selected AI-DLC or Superpowers workflow position, evidence, exact next entry, and uncertainty without changing the project.
user-invocable: true
---
...
exactly `claude`; `workflow` must be `aidlc` or `superpowers` and present in
...
Next Skill/entry: <exact Claude Code invocation, or Unverified>
...
does not prove phase completion. Choose the mapped Claude Code `/superpowers:...`
entry only when the evidence is unambiguous; otherwise set
`Next Skill/entry: Unverified`.
