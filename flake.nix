    ...
    let
      ...
      mkWorkflowCli =
        ...
        let
          ...
          inceptionCli = pkgs.writeShellApplication {
            name = "agent-workflow-inception";
            runtimeInputs = [ python pkgs.git ];
            text = ''
              export PYTHONPATH="${self}/agent-workflows/src''${PYTHONPATH:+:$PYTHONPATH}"
              exec python -m ai_agent_workflow.inception_cli "$@"
            '';
          };
        ...
        pkgs.symlinkJoin {
          ...
          paths = [ workflowCli inceptionCli configCli implementationStatusCli ];
        };
    in
    {
